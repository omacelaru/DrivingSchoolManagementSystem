package com.drivingschool.payment.security;
...
public class PaymentAuthorizationService {
    ...
    public boolean canAccessPayment(Long paymentId, Authentication authentication) {
        if (isAdminOrService(authentication)) {
            return true;
        }
        if (!isStudent(authentication)) {
            return false;
        }
        Long studentId = profileId(authentication);
        if (studentId == null) {
            return false;
        }
        return paymentRepository.findById(paymentId)
                .map(Payment::getStudentId)
                .map(ownerId -> Objects.equals(ownerId, studentId))
                .orElse(false);
    }
